...
export default function StatsView() {
  ...
  const [gloveError, setGloveError] = useState("");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/season-players?season_id=${selectedSeason}`).then(res => res.json())
    ])
      .then(([scorersData, glovesData, playersData]) => {
        ...
        const playerMap: Record<string, PlayerOption> = {};
  ...
  const handleAddScorer = async () => {
    setScorerError("");
    try {
      if (!newScorerForm.rank || !newScorerForm.player_id) {
        setScorerError("Rank and Player are required");
        return;
      }
      const payload = {
        rank: newScorerForm.rank,
        player_id: newScorerForm.player_id,
        goals: newScorerForm.goals || 0,
        assists: newScorerForm.assists || 0,
      ...
      setScorerError(err.message || "Failed to add scorer");
    }
  };
  ...
  const handleAddGlove = async () => {
    setGloveError("");
    try {
      if (!newGloveForm.rank || !newGloveForm.player_id) {
        setGloveError("Rank and Player are required");
        return;
      }
      const payload = {
        rank: newGloveForm.rank,
        player_id: newGloveForm.player_id,
        clean_sheets: newGloveForm.clean_sheets || 0,
        saves: newGloveForm.saves || 0,
        season_id: selectedSeason
      };
      const res = await fetch(`/api/golden-gloves`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
      ...
      setGloveError(err.message || "Failed to add golden glove");
    }
  };
        ...
        <section>
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-4">
              ...
              {scorerError && <span className="text-red-500 text-xs">{scorerError}</span>}
            </div>
            {!isAddingScorer && (
              <button
                  ...
                  <tr className="bg-stone-900/60 border-t border-stone-800">
                    ...
                    <td className="px-5 py-4">
                      ...
                      >
                        ...
                        {(Object.values(players) as PlayerOption[]).map(p => (
                          <option key={p.id} value={p.id}>{p.player_profile?.name || p.id}</option>
                        ))}
                      </select>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex justify-end gap-3">
                        ...
                        <button onClick={() => { setIsAddingScorer(false); setNewScorerForm({}); setScorerError(""); }} className="text-stone-500 hover:text-stone-300">Cancel</button>
                      </div>
                    </td>
                  </tr>
                )}
        ...
        <section>
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-4">
              ...
              {gloveError && <span className="text-red-500 text-xs">{gloveError}</span>}
            </div>
            {!isAddingGlove && (
              <button
                  ...
                  <tr className="bg-stone-900/60 border-t border-stone-800">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex justify-end gap-3">
                        ...
                        <button onClick={() => { setIsAddingGlove(false); setNewGloveForm({}); setGloveError(""); }} className="text-stone-500 hover:text-stone-300">Cancel</button>
                      </div>
                    </td>
                  </tr>
                )}
